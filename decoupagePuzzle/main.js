...
// create div parent
var parentPuzzle = document.createElement("div");
parentPuzzle.id = "parentPuzzle";
parentPuzzle.style.width = "100%";
parentPuzzle.style.height = "100%";
// add div parent to body
...
function randomize() {
  for (var i = 1; i < puzzle.listPieces.length; i++) {
    ...
    var randomY =
      Math.floor((Math.random() * HEIGHT) / DIFFICULTE) * DIFFICULTE;
...
function onDragEnd() {
  ...
  this.dragging = false;
  if (this.data != null) {
    var newPosition = this.getGlobalPosition();
...
function onDragMove() {
  if (this.dragging) {
    ...
    console.log(this.parent.getLocalBounds());
    console.log(this.data.global);
    var newPosition = this.parent.toLocal(this.data.global);
...
for (var i = 1; i < puzzle.listPieces.length; i++) {
  ...
  puzzle.listPieces[id].sprite.on("pointerup", () => {
    ...
    console.log(
      puzzle.listPieces[id].sprite.getGlobalPosition().x * app.stage.scale.x
    );
...
var rectSprite = PIXI.Sprite.from("sampe1.png");
...
function testWin() {
  ...
  for (var i = 1; i < puzzle.listPieces.length; i++) {
    if (
      puzzle.listPieces[i].x == puzzle.listPieces[i].originalX &&
      puzzle.listPieces[i].y == puzzle.listPieces[i].originalY
    ) {
      pieceOK++;
    }
